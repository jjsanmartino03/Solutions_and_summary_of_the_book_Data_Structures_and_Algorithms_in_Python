...
def oper_order(
    opers,
):  # Función que ordena los operadores usados, basandose en la precedencia de operadores (primero la potenciación, después la multiplicación o división y por último la suma y resta, que, al ser consideradas iguales, se ordenan según como estaban ubicadas)
    ...
    for i in opers:
        real_ord.append(
            i
        )  # se agregan el + y el - en el order en el q estaban (de izq a der)
    return real_ord
...
def check_special(data, used):
    specials = {"√": root}
    for value in data:
        if not value.isnumeric() and not value in used:
            current = ""
            num = ""
            for i in value:
                if not i.isdecimal() and i != ".":
                    ...
                    num += i
            result = specials[current](num)
            data[data.index(value)] = str(result)
    return None
...
if __name__ == "__main__":
    ans = 0
    print(
        "To exit the calculator write 'quit()\nThe non-basic commands are 'ans' and '√'"
    )
    while True:
        a = input(">>> ")
        if a != "quit()" and a:
